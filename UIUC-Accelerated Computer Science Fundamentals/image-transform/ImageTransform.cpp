...
PNG createSpotlight(PNG image, int centerX, int centerY) {
  ...
  for (int x = 0; x < image.width(); x++) {
    for (int y = 0; y < image.height(); y++) {
      ...
      HSLAPixel& pixel = image.getPixel(x, y);
      if (distance > 160) pixel.l *= 0.2;
      else pixel.l *= 1 - distance * 0.005;
    }
  }
  ...
  for (int x = 0; x < image.width(); x++) {
    for (int y = 0; y < image.height(); y++) {
      ...
      if (pixel.h != 11 && pixel.h != 216) {
        if (pixel.h < 113.5 || pixel.h > 295.5) pixel.h = 11;
        else pixel.h = 216;
      }
    }
  }
  ...
  for (int x = 0; x < secondImage.width(); x++) {
    for (int y = 0; y < secondImage.height(); y++) {
      ...
      if (pixel.l == 1) {
        ...
        if (pixelChange.l + 0.2 >= 1) pixelChange.l = 1;
        else pixelChange.l += 0.2;
      }
    }
  }
