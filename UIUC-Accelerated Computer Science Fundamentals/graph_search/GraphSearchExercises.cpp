...
int GridGraph::countEdges() const {
  ...
  // =======================================================================
  // TODO: Your code here!
  unordered_set<IntPairPair> edges;
  unordered_set<IntPair> points;
  for (const auto& t : adjacencyMap) {
    const auto& p1 = t.first;
    const auto& p1_neighbors = t.second;
    if (p1_neighbors.empty()) points.insert(p1);
    else {
      for (const auto& p2 : p1_neighbors) {
        IntPairPair edge;
        if (p1 < p2) edge = make_pair(p1,p2);
        else edge = make_pair(p2,p1);
        edges.insert(edge);
      }
    }
  }
